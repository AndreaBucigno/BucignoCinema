...
$clienti = $pdo->query("SELECT * FROM cliente ORDER BY cognome")->fetchAll(PDO::FETCH_ASSOC);
...
foreach ($clienti as $c) {
  ...
  $righe .= "
    <tr>
        <td>{$c['id']}</td><td>{$nome}</td><td>{$cognome}</td><td>{$nascita}</td>
        <td>
            <button class='btn btn-sm btn-primary' data-bs-toggle='modal' data-bs-target='#editClienteModal'
                data-id='{$c['id']}' data-nome='{$nome}' data-cognome='{$cognome}' data-nascita='{$nascita}'>
...
<div class='modal fade' id='addClienteModal' tabindex='-1'>
  <div class='modal-dialog'><div class='modal-content bg-dark border-danger'>
    ...
    <div class='modal-footer bg-dark border-danger'><button type='button' class='btn btn-outline-secondary' data-bs-dismiss='modal'>Annulla</button><button type='submit' form='addClienteForm' class='btn btn-danger fw-semibold'><i class='bi bi-check-circle me-1'></i>Salva</button></div>
  </div></div>
</div>
...
<div class='modal fade' id='editClienteModal' tabindex='-1'>
  <div class='modal-dialog'><div class='modal-content bg-dark border-warning'>
    ...
    <div class='modal-footer bg-dark border-warning'><button type='button' class='btn btn-outline-secondary' data-bs-dismiss='modal'>Annulla</button><button type='submit' form='editClienteForm' class='btn btn-warning text-dark fw-semibold'><i class='bi bi-check-circle me-1'></i>Salva</button></div>
  </div></div>
</div>
...
<div class='container-fluid text-white p-4'>
    <div class='d-flex justify-content-between align-items-center mb-4'>
        ...
        <button class='btn btn-danger' data-bs-toggle='modal' data-bs-target='#addClienteModal'><i class='bi bi-plus-circle me-1'></i>Aggiungi Cliente</button>
    </div>
